...
// src/lib/permissions's getPermissions()/canManageCharacter() and most
// pages call this once per request.
//
// Preview-aware: every capability check in src/lib/permissions ultimately
// runs through this function, always with the *acting* user's own id
// (never a target user's), so layering the view-as override here — rather
// than touching each call site — is sufficient to make a preview airtight:
// an admin viewing as "member" really can't reach admin actions, not just
// the admin nav link. Only ever narrows a real admin's own effective role
// for the current request; can never widen anyone else's, since it only
// applies when the freshly-read DB role is "admin".
...
// leader-tier capability already treats them as equivalent (roleCan() in
// src/lib/permissions/capabilities.ts special-cases "admin" to always pass).
// "Does the guild have leadership yet" checks need the same equivalence:
// three call sites used to query
