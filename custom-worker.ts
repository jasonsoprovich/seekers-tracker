...
//
// No Next Route Handler calls this Durable Object. Keep every new live-bid
// RPC in this wrapper too; see CLAUDE.md's OpenNext loopback warning.
...
import { roleCan } from "./src/lib/permissions/capabilities";
import { loadPermissionMatrix } from "./src/lib/permissions/db";
import { effectiveRoleWithViewAs } from "./src/lib/view-as";
...
// GET/POST /api/live-bids/config — browser-session control for leaders.
// This stays in the custom Worker because it calls the auction DO. The role
// is read fresh from D1 and narrowed by the same admin view-as cookie used by
// Next's getUserRole(), so previewing member/officer cannot mutate config.
async function handleLiveBidsConfig(request: Request, env: CloudflareEnv): Promise<Response> {
  const auth = createAuth(env, cfOf(request));
  const session = await auth.api.getSession({ headers: request.headers });
  if (!session) return Response.json({ error: "Not signed in." }, { status: 401 });

  const db = drizzle(env.DATABASE, { schema });
  const [memberAllowed, roleRow] = await Promise.all([
    fetchIsMemberAllowed(db, session.user.id),
    db.select({ role: users.role }).from(users).where(eq(users.id, session.user.id)).then((rows) => rows[0]),
  ]);
  if (!memberAllowed) return Response.json({ error: "Not allowed." }, { status: 403 });

  const realRole = roleRow?.role ?? null;
  const effectiveRole = effectiveRoleWithViewAs(realRole, request.headers.get("cookie"));
  const matrix = await loadPermissionMatrix(db);
  if (!roleCan(matrix, effectiveRole, "epgp.liveBids.visibility")) {
  ...
  try {
    const response =
      request.method === "GET"
        ? await stub.fetch("https://live-auction-session/config")
        : await stub.fetch("https://live-auction-session/config", {
            method: "POST",
...
// Correlation ID (remediation plan Phase 0.1, 2026-09-12). Concurrent
// requests in the same isolate interleave their [hang]/[slow]/[perf] log
// lines in Workers Logs with nothing tying a given "stage getSession still
// running" line back to the specific request/navigation it belongs to.
// Stamping every request with a short id here — read back inside
// src/lib/session.ts via headers() and folded into its timed() stage
// labels — makes those lines greppable as one request's timeline instead
// of a jumble.
...
export default {
  async fetch(request, env, ctx) {
    ...
    ctx.waitUntil(checkAsyncContextImplementation());

    // See REQUEST_ID_HEADER's comment. An incoming Request's Headers are
    // immutable in Workers (confirmed against `wrangler dev`: `.set()`
    // directly throws "Can't modify immutable headers") — clone the
    // request with the header added instead, same pattern this file
    // already uses to forward a request into the live-bids DO. Reassigning
    // `request` (not a new local name) so every downstream use in this
    // function — canonicalRedirect, the live-bids handlers, handler.fetch —
    // picks up the id-bearing request without its own edit.
    if (!request.headers.has(REQUEST_ID_HEADER)) {
      const withReqId = new Headers(request.headers);
      withReqId.set(REQUEST_ID_HEADER, crypto.randomUUID().slice(0, 8));
      // Constructing from an existing Request carries its `cf` object over
      // too (Fetch spec) — the cast is only needed because Workers' own
      // Request<> type is generic over an exact CfProperties shape the
      // constructor's return type doesn't restate.
      request = new Request(request, { headers: withReqId }) as typeof request;
    }
    ...
    if (url.pathname === "/api/live-bids/config" && (request.method === "GET" || request.method === "POST")) {
      return handleLiveBidsConfig(request, env);
    }
    ...
    const deadline = deadlineMs
      ? new Promise<Response>((resolve) => {
          deadlineTimer = setTimeout(() => {
            ...
            console.warn(`[hang] [${reqId}] ${request.method} ${url.pathname} no response after ${deadlineMs}ms; returning 503${rsc}`);
    ...
    try {
      ...
      status = resp.status;
      return deadline ? withBodyDeadline(resp, `[${reqId}] ${request.method} ${url.pathname}${request.headers.has("RSC") ? " rsc" : ""}`) : resp;
    } catch (e) {
      status = -1;
      throw e;
    } finally {
      ...
      if (perfEnabled()) console.log(`[perf] [${reqId}] request ${request.method} ${url.pathname} ${ms}ms`);
      if (ms >= SLOW_REQUEST_MS) {
        const hasCookie = (request.headers.get("cookie") ?? "").includes("better-auth.session");
        const rsc = request.headers.has("RSC") ? " rsc" : "";
        console.warn(`[slow] [${reqId}] ${request.method} ${url.pathname} ${ms}ms status=${status} session=${hasCookie ? "yes" : "no"}${rsc}`);
      }
    }
  },
